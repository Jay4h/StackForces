...
Napi::String GenerateDID(const Napi::CallbackInfo &info) {
  ...
  // CRITICAL: This value must remain constant in production
  // Changing it will invalidate ALL existing DIDs across the entire system
  // Currently matches DID_SALT environment variable in backend/.env
